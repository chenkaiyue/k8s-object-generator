...
func RegisterGroupVersionGo(gv schema.GroupVersion, args *gargs.GeneratorArgs, customArgs *args.CustomArgs) generator.Generator {
...
func (f *registerGroupVersionGo) Init(c *generator.Context, w io.Writer) error {
	...
	for _, t := range types {
		m := map[string]interface{}{
			...
			"plural": GuessPluralName(strings.ToLower(t.Name.Name)),
		}
