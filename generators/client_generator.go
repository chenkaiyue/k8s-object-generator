...
func (cg *ClientGenerator) Packages(context *generator.Context, arguments *gargs.GeneratorArgs) generator.Packages {
	customArgs := arguments.CustomArgs.(*args.CustomArgs)
	...
	for gv, t := range customArgs.TypesByGroup {
		if !groups[gv.Group] {
			if generateTypesGroups[gv.Group] {
				packageList = append(packageList, cg.typesGroupPackage(t[0], gv, arguments, customArgs))
			}
		}
		groups[gv.Group] = true
		if generateTypesGroups[gv.Group] {
			packageList = append(packageList, cg.typesGroupVersionPackage(t[0], gv, arguments, customArgs))
			packageList = append(packageList, cg.typesGroupVersionDocPackage(t[0], gv, arguments, customArgs))
		}
	}
...
func (cg *ClientGenerator) typesGroupPackage(name *types.Name, gv schema.GroupVersion,
	generatorArgs *gargs.GeneratorArgs, customArgs *args.CustomArgs) generator.Package {
...
func (cg *ClientGenerator) typesGroupVersionDocPackage(name *types.Name, gv schema.GroupVersion,
	generatorArgs *gargs.GeneratorArgs, customArgs *args.CustomArgs) generator.Package {
...
func (cg *ClientGenerator) typesGroupVersionPackage(name *types.Name, gv schema.GroupVersion, generatorArgs *gargs.GeneratorArgs, customArgs *args.CustomArgs) generator.Package {
	packagePath := name.Package
	return Package(generatorArgs, packagePath, func(context *generator.Context) []generator.Generator {
		return []generator.Generator{
			RegisterGroupVersionGo(gv, generatorArgs, customArgs),
